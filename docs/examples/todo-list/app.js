...
(function(exports) {
    ...
    <li class="${todo.done ? "completed" : ""}">
      <div class="view">
        ...
        <label ondblclick="${startEditItem}(this)">${todo.text}</label>

        <button class="destroy"
                onclick="${todoFunc(() => model.remove(todo))}()"></button>
      </div>
      <input class="edit"
             onblur="${endEditItem}(this)"
  ...
  <button class="clear-completed ${model.completed().length ? "" : "hidden"}" 
    ...
    const FilterList = () => `
    ${FilterItem("#/", VISIBILITY.ALL)}
    ${FilterItem("#/active", VISIBILITY.ACTIVE)}
    ${FilterItem("#/completed", VISIBILITY.COMPLETED)}
  `;
